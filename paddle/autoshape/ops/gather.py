...
def transpose_and_gather_feat(feat, ind):
    ...
    Returns:
        feat: indexed feature, batch * max_num_objects * channel
    """
    feat = paddle.transpose(feat, perm=[0, 2, 3, 1])
    feat = paddle.reshape(feat, (feat.shape[0], -1, feat.shape[3]))
...
def gather_feat(feat, ind):
    ...
    batch = ind.shape[0]
    num_obj = ind.shape[1]
    channel = feat.shape[2]
    ind = paddle.unsqueeze(ind, axis=2)
    ind = paddle.expand(ind, (batch, num_obj, channel))  # batch * channel * num_obj
