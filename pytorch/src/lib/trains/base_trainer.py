...
class BaseTrainer(object):
  def __init__(
    ...
    self.rampup = exp_rampup(opt.exp_rampup)
    self.rampup_prob = exp_rampup(opt.exp_rampup)
    self.rampup_coor = exp_rampup(opt.exp_rampup)
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      loss = loss['hm_loss'].mean() * opt.hm_weight + \
             ...
             loss['p3d_loss'].mean() * opt.dim_weight
