...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--input_images', default='',
                                 help='path to input_images')
        ...
        self.parser.add_argument('--num_joints', type=int, default=57,
                                 help='number of keypoints.')
        self.parser.add_argument('--is_use_corner', type=int, default=True,
                                 help='is_use_corner.')

        self.parser.add_argument('--sample_pc', type=int, default=48,
                                 help='is_use_corner.')
        # input
        ...
        self.parser.add_argument('--trainval', action='store_true',
                                 help='include validation in training and '
                                      'test on test set')
        self.parser.add_argument('--exp_rampup', type=int, default=100,
                                 help='max exp_rampup')
        ...
        self.parser.add_argument('--task', type=str, default='my3dtask')
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        opt.heads = {
            'hm': dataset.num_classes,
            'wh': 2,
            'hps': opt.num_joints * 2,
            'rot': 8,
            'dim': 3,
            'p3d': opt.num_joints * 3,
            'adaptive_weights': opt.num_joints * 2,
            'prob': 1
        }
    ...
    def init(self, args=''):
        ...
        default_dataset_info = {
            ...
            'AutoShape_kitti': {
                ...
                'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225],
                'dataset': 'kitti_hp', 'num_joints': opt.num_joints,
                'flip_idx': [[i, i + 1] for i in range(0, opt.num_joints - 1, 2)]},
            'AutoShape_nuscenes': {
                'default_resolution': [896, 1600], 'num_classes': 10,
                'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225],
                'dataset': 'nuscenes_hp', 'num_joints': opt.num_joints,
                'flip_idx': [[i, i + 1] for i in range(0, opt.num_joints - 1, 2)]},
        }
        ...
        dataset = Struct(default_dataset_info['AutoShape_kitti'])
