...
class AutoShapeDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        p3d = np.zeros((self.max_objs, self.num_joints * 3), dtype=np.float32)
        # p3ds_mask = np.zeros((self.max_objs, self.num_joints * 3), dtype=np.int64)

        calib = np.array(
            [7.215377000000e+02, 0.000000000000e+00, 6.095593000000e+02, 4.485728000000e+01, 0.000000000000e+00,
             7.215377000000e+02, 1.728540000000e+02, 2.163791000000e-01, 0.000000000000e+00, 0.000000000000e+00,
             1.000000000000e+00, 2.745884000000e-03], dtype=np.float32)
        ...
        for k in range(num_objs):
            ...
            pts = np.array(ann['keypoints'][:num_joints*3], np.float32).reshape(num_joints, 3)
            alpha1=ann['alpha']
            orien=ann['rotation_y']
            loc = ann['location']
            p3ds = np.array(ann['p3d'], np.float32).reshape(num_joints, 3)
            ...
            if flipped:
                ...
                for e in self.flip_idx:
                    ...
                    # p3ds[e[0]], p3ds[e[1]] = p3ds[e[1]].copy(), p3ds[e[0]].copy()
            ...
            if (h > 0 and w > 0) or (rot != 0):
                ...
                if coor_kps_mask[k,(num_joints-1)*2]==0 or coor_kps_mask[k,(num_joints-1)*2+1]==0:
