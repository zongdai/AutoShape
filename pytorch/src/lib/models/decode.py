...
def autoshape_gen_position(kps, dim, p3d, adaptive_weights, rot,meta, const, is_use_corner=True):
    b=kps.size(0)
    c=kps.size(1)
    opinv=meta['trans_output_inv']
    calib=meta['calib']
    num_joints = kps.size(2) // 2
    opinv = opinv.unsqueeze(1)
    opinv = opinv.expand(b, c, -1, -1).contiguous().view(-1, 2, 3).float()
    kps = kps.view(b, c, -1, 2).permute(0, 1, 3, 2)
    hom = torch.ones(b, c, 1, num_joints).cuda() #### !!! ####
    kps = torch.cat((kps, hom), dim=2).view(-1, 3, num_joints) #### !!! ###
    kps = torch.bmm(opinv, kps).view(b, c, 2, num_joints) #### !!! ###
    kps = kps.permute(0, 1, 3, 2).contiguous().view(b, c, -1)  # 16.32,18
    si = torch.zeros_like(kps[:, :, 0:1]) + calib[:, 0:1, 0:1]
    alpha_idx = rot[:, :, 1] > rot[:, :, 5]
    alpha_idx = alpha_idx.float()
    ...
    #### !!! ### center
    rot_y = alpna_pre + torch.atan2(kps[:, :, (num_joints-1) * 2:(num_joints-1)*2 + 1] - calib[:, 0:1, 2:3], si)
    rot_y[rot_y > np.pi] = rot_y[rot_y > np.pi] - 2 * np.pi
    rot_y[rot_y < - np.pi] = rot_y[rot_y < - np.pi] + 2 * np.pi
    ...
    calib = calib.expand(b, c, -1, -1).contiguous()
    #### !!! ###
    kpoint = kps[:, :, :(num_joints)*2]
    f = calib[:, :, 0, 0].unsqueeze(2)
    f = f.expand_as(kpoint)
    cx, cy = calib[:, :, 0, 2].unsqueeze(2), calib[:, :, 1, 2].unsqueeze(2)
    cxy = torch.cat((cx, cy), dim=2)
    #### !!! ###
    cxy = cxy.repeat(1, 1, (num_joints))  # b,c,(num_points - 1) * 2
    kp_norm = (kpoint - cxy) / f
    ...
    #### !!! ###
    if is_use_corner:
        for i in range((num_joints - 9) * 2):
            B[:, :, i * 2:i * 2 + 1] = p3d[:, :, i * 3 + 0:i * 3 + 1] * cosori + p3d[:, :, i * 3 + 2:i * 3 + 3] * sinori
    ...
    # AT = A.permute(0, 1, 3, 2)
    # AT = AT.view(b * c, 3, 2 * (num_joints-1))
    A = A.view(b * c, 2 * (num_joints), 3)
    B = B.view(b * c, 2 * (num_joints), 1).float()
    # mask = mask.unsqueeze(2)

    # ======Add Adaptive Weights=======
    adaptive_weights = torch.sigmoid(adaptive_weights)
    # adaptive_weights = adaptive_weights[:, :, 0:adaptive_weights.shape[2] - 2]
    adaptive_weights_diag = torch.diag_embed(adaptive_weights)  # b x n x (num_joints * 2) x (num_joints * 2)
    adaptive_weights_diag = adaptive_weights_diag.view(b * c, (num_joints) * 2, (num_joints) * 2)
    ...
    # print(pinv)
    # pinv[:, :, 1] = pinv[:, :, 1] -  dim[:, :, 0] / 2
    return pinv, rot_y, kps, adaptive_weights


def autoshape_pose_decode(
        heat, kps,dim,rot, p3d, adaptive_weights, prob,K=100,meta=None,const=None, is_use_corner=True):
    ...
    dim = _transpose_and_gather_feat(dim, inds)
    dim = dim.view(batch, K, 3)
    p3d = _transpose_and_gather_feat(p3d, inds)
    p3d = p3d.view(batch, K, -1)
    adaptive_weights = _transpose_and_gather_feat(adaptive_weights, inds)
    adaptive_weights = adaptive_weights.view(batch, K, -1)
    # dim[:, :, 0] = torch.exp(dim[:, :, 0]) * 1.63
    # dim[:, :, 1] = torch.exp(dim[:, :, 1]) * 1.53
    # dim[:, :, 2] = torch.exp(dim[:, :, 2]) * 3.88
    rot = _transpose_and_gather_feat(rot, inds)
    ...
    prob = _transpose_and_gather_feat(prob, inds)[:,:,0]
    prob = prob.view(batch, K, 1)
    position,rot_y,kps_inv, adaptive_weights = autoshape_gen_position(kps,dim,p3d,adaptive_weights, rot,meta,const, is_use_corner=is_use_corner)
    #bboxes=kps[:,:,0:4]
    bboxes_kp=kps.view(kps.size(0),kps.size(1),num_joints,2)
    box_min,_=torch.min(bboxes_kp,dim=2)
    box_max,_=torch.max(bboxes_kp,dim=2)
    bboxes=torch.cat((box_min,box_max),dim=2)
    hm_score=kps[:,:,0:num_joints]
    detections = torch.cat([bboxes, scores, kps_inv, dim, hm_score, rot_y, position, prob, clses, p3d, adaptive_weights], dim=2)
